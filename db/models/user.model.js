...
//* Role-based phone validation
userSchema.path("phone").validate(function (value) {
  if (this.role === "admin" && value) {
    return false;
  }
  return true;
}, "Admins cannot have a phone number");

//* Role-based phone verification flag validation
userSchema.path("isPhoneVerified").validate(function (value) {
  if (this.role === "admin" && value === true) {
    return false;
  }
  return true;
}, "Admins cannot have a verified phone number");
